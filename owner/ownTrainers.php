...
<body>
            ...
            <div class="container-fluid">
                <div class="navbar-collapse" id="navbarSupportedContent">
                    <ul class="navbar-nav me-auto mb-2 mb-lg-0">
                        <li class="nav-item">
                            <a class="nav-link active" aria-current="page" href="ownTrainReg.php">
                                <button type="button" class="btn btn-dark">Add New Trainer</button></a>
                        </li>
                    </ul>
                    <form class="d-flex" action="" method="post" >
                        <input class="form-control me-2" type="search" name="searchTrainer" autocomplete="off" placeholder="Search by phone/name"
                            aria-label="Search">
                        <button class="btn btn-outline-success" type="submit" name="searchTrain">Search</button>
                    </form>
                </div>
            </div>
        </nav>
    ...
    <section class="container-fluid my-2 w-100">
        ...
        <div class="container-fluid bg-dark bg-opacity-10 shadow">
            <h2 class="text-center py-3 mx-auto plans">Our Trainers</h2>
            <div class="row gy-5">
                <?php
                    
                    $tablename = $_SESSION['email'] ."_trains";
                    $flag = 0;
                    if(isset($_POST['searchTrain']))
                    {
                        $temp = mysqli_real_escape_string($con, $_POST['searchTrainer']);
                   ...
                   // $selectquery = " select * from `$tablename` ";
                    ...
                    else
                    {
                        ...
                        {
                            $newDate = date("d-m-Y", strtotime($res['date']));
                            ?>
                            <div class="col-sm-10 col-md-4 col-lg-4 mx-auto ">
                                <div class="card mx-auto" style="width: 18rem;">
                                    <div class="card-body">
                                        ...
                                        <p class="card-text"><i class="fa fa-regular fa-table"
                                                style="font-size: 1.2rem; margin-right: 0.5rem;"></i><?php echo $res['type'] ?></p>
                                                <?php
                                        ...
                                        <p class="card-text" style="background-color: green;"><i class="fa fas fa-credit-card"
                                                style="font-size: 1.2rem; margin-right: 0.5rem;"></i><span class="status"
                                                style="color: white;"><?php echo $res['status'] . " on " .  $newDate ?></span></p>
                                                <?php
                                        }
                                        ?>
                                         <div class="d-flex justify-content-around">
                                            <a href="ownTrainEdit.php?id=<?php echo $res['id']; ?>" class="edit1 card-link col-2 text-center" data-bs-toggle="tooltip"
                                                data-bs-placement="top" title="Edit info"><i class="fa fas fa-edit"
                                                    style="font-size: 2rem;"></i></a>
                                            <a href="ownTrainDel.php?id=<?php echo $res['id']; ?>" class="edit2 card-link col-2 text-center" data-bs-toggle="tooltip"
                    ...
                    if($flag == 1)
                    {
                        ?>
                        <p class="text-center mx-auto my-5">
                                <a class="nav-link active" aria-current="page" href="ownTrainers.php">
                                    <button type="button" class="btn btn-dark">See all Trainers</button></a>
                                </p>
                                <?php
                    }
                ?>
